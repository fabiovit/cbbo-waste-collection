...
def _calvisano(d: date):
    ...
    if wd==1: t.extend((PLASTIC,GLASS_CANS))
    if wd==2: t.append(PAPER)
    if wd==3: t.extend((RESIDUAL,SANITARY))
    if wd==4: t.append(ORGANIC)
    return t
...
def _carpenedolo(d: date):
    ...
    if wd==4: t.extend((ORGANIC,SANITARY,GLASS_CANS))
    return t
...
def _castenedolo(d: date):
    ...
    if wd==1:
        ...
        if 6 <= d.month <= 8: t.append(SANITARY)
    if wd==2: t.append(PAPER)
...
def _flero(d: date):
    # 2026 calendar: Mon organic/green, Tue plastic, Wed paper (+ sanitary),
    # Thu residual, Fri organic + glass.
    t=[]; wd=d.weekday()
    if wd==0:
        t.append(ORGANIC)
        if 3 <= d.month <= 11: t.append(GREEN)
    if wd==1: t.append(PLASTIC)
    if wd==2: t.extend((PAPER,SANITARY))
    if wd==3: t.append(RESIDUAL)
    if wd==4: t.extend((ORGANIC,GLASS_CANS))
    return t
...
def _nuvolera(d: date):
    ...
    if wd==0:
        ...
        if 3 <= d.month <= 11: t.append(GREEN)
    if wd==1: t.append(ORGANIC)
    if wd==2 and _biweekly(d,date(2026,7,1)): t.append(PAPER)
    if wd==3: t.append(PLASTIC)
    if wd==4: t.extend((ORGANIC,SANITARY))
    if wd==5 and 6 <= d.month <= 8: t.append(ORGANIC)
    if wd==6 and _biweekly(d,date(2026,7,5)): t.append(GLASS_CANS)
    return t
